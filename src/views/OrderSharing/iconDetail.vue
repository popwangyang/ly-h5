  ...
  <div class="orderktvDetail">
    <ContentLoad class="orderktv-wrapper" :getInfo="getData">
      ...
      <van-cell-group class="group1" key="group1">
        ...
        <van-cell
          title-class="detailaArea"
          title="地区"
          :value="res.area_display"
        />
        <div v-if="type === 'ktv'">
          <van-cell title="支付方式">
          ...
          <van-cell
            title="曲库服务费"
            :value="`￥${toFixed2(res.song_service_fee_display)}`"
          />
          <van-cell
            title="综合技术服务费"
            :value="`￥${toFixed2(res.technology_service_fee_display || 0)}`"
          />
          <van-cell
            title="支付通道费率"
            :value="payment_channel_rate_display"
          />
        </div>
        <div v-else>
          <van-cell
          ...
          <van-cell
            title="支付通道费率"
            :value="payment_channel_rate_display || 0"
          />
        </div>
      </van-cell-group>

<template>
  <div class="implementationBox">
    <ContentLoad :getInfo="getData">
      <!-- <van-cell title="实施方式" value-class="cellValue" :value="formData.way" /> -->
      <span slot="empty">
      ...
      <van-cell
        ...
        :value="formData.communication_mode | communicationModeFilter"
      />
...
export default {
  ...
  computed: {
    ...
    hasQR() {
      if (
        this.$store.state.ktv.QRCode.qr_tv &&
        this.$store.state.ktv.QRCode.qr_tv.length == 1
      ) {
        return true;
      } else {
        return false;
  ...
  methods: {
    ...
    getData() {
      return new Promise((resolve, reject) => {
        ...
        this.ImplementationInfo(send_data)
          .then(res => {
            if (res != "") {

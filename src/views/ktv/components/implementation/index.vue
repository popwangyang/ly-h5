<template>
  <div>
    <ContentLoad :getInfo="getData">
      <cell-selected title="实施方式" :dataList="ways" v-model="formData.way" />
      ...
      <van-cell title="KTV直连ID" value-class="cellValue" :value="formData.ktvzlid" />
      <span v-show="formData.communication_mode == 2">
        <cell-input
          title="VOD Sever地址"
          :required="false"
          :maxlength="100"
          v-model="formData.vod_server"
        />
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        vod: [{ required: true, message: "VOD品牌不能为空" }],
        box: [{ required: true, validate: boxValidate }],
        version: [{ required: true, message: "软件系统版本号不能为空" }],
        ...
        communication_mode: [{ required: true, message: "通讯模式不能为空" }]
      }
    };
  },
  watch: {
  ...
  methods: {
    btnClick() {
      formValidate(this.formData, this.rules).then(val => {
        if (val) {
          ...
          if (this.isEdited) {
            editedImplementation(send_data, this.formData.id)
              .then(res => {
    ...
    getData() {
      ...
      return new Promise((resolve, reject) => {
        getImplementationDetail(send_data)
          .then(res => {
